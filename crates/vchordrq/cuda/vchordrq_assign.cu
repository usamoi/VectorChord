...
static_assert(alignof(cudaIpcMemHandle_t) == 1);
...
int QUOTE(server_addr)(server_t *server, op_t *op, size_t *d, size_t *n,
                         void **centroids_0, cudaIpcMemHandle_t *centroids_1) {
  if (cudaDeviceSynchronize() != cudaSuccess) {
    return -1;
  }
  *op = server->op;
  *d = server->d;
  *n = server->n;
  *centroids_0 = server->centroids;
  if (cudaIpcGetMemHandle(centroids_1, server->centroids) != cudaSuccess) {
    return -1;
  }
  return 0;
}

client_t *QUOTE(client_alloc_0)(op_t op, size_t d, size_t n, void *centroids_0,
                                size_t m) {
  if (cudaDeviceSynchronize() != cudaSuccess) {
    return NULL;
  }
...
client_t *QUOTE(client_alloc_1)(op_t op, size_t d, size_t n,
                                cudaIpcMemHandle_t centroids_1, size_t m) {
  if (cudaDeviceSynchronize() != cudaSuccess) {
    return NULL;
  }
  client_t *client = (client_t *)malloc(sizeof(client_t));
  ...
  client->buffer = NULL;
  client->results = NULL;
  if (cudaIpcOpenMemHandle(&client->centroids, centroids_1,
                           cudaIpcMemLazyEnablePeerAccess) != cudaSuccess) {
    free(client);
    return NULL;
  }
  if (cudaMalloc(&client->vectors, m * d * size(op)) != cudaSuccess) {
    cudaIpcCloseMemHandle(client->centroids);
    free(client);
    return NULL;
  }
  if (cudaMalloc(&client->buffer, m * n * size(op)) != cudaSuccess) {
...
int QUOTE(client_query)(client_t *client, size_t k, void *vectors,
  ...
  if (cudaMemcpy(client->vectors, vectors, k * client->d * size(client->op),
                 cudaMemcpyHostToDevice) != cudaSuccess) {
    return -1;
  }
  switch (client->op) {
  case vecf32_dot:
    QUOTE(kernel_0)<<<blocks, threads>>>((fp32 *)client->centroids, client->d,
                                         client->n, (fp32 *)client->vectors,
                                         client->results, k);
    break;
  case vecf32_l2s:
    QUOTE(kernel_1)<<<blocks, threads>>>((fp32 *)client->centroids, client->d,
                                         client->n, (fp32 *)client->vectors,
                                         client->results, k);
    break;
  case vecf16_dot:
    QUOTE(kernel_2)<<<blocks, threads>>>((fp16 *)client->centroids, client->d,
                                         client->n, (fp16 *)client->vectors,
                                         client->results, k);
    break;
  case vecf16_l2s:
    QUOTE(kernel_3)<<<blocks, threads>>>((fp16 *)client->centroids, client->d,
                                         client->n, (fp16 *)client->vectors,
                                         client->results, k);
    break;
  default:
  ...
  if (cudaMemcpy(results, client->results, k * sizeof(uint32_t),
