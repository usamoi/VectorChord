    ...
    R::Page: Page<Opaque = Opaque>,
{
    ...
    let mut step = |state: State| {
        ...
        for (Reverse(dis_f), AlwaysEqual(norm), AlwaysEqual(first)) in state {
            tape::read_h1_tape::<R, _, _>(
                by_next(index, first),
                || O::block_access(&lut.0, is_residual, dis_f.to_f32(), norm, epsilon),
                |lowerbound, head, norm, first, prefetch| {
                    results.push((
                        Reverse(lowerbound),
    ...
    for (Reverse(dis_f), AlwaysEqual(norm), AlwaysEqual(first)) in state {
        ...
        let mut callback = id_2(|lowerbound, head, payload, prefetch| {
            results.push((
        ...
        tape::read_appendable_tape::<R, _>(
            ...
            O::binary_access(&lut.1, is_residual, dis_f.to_f32(), norm, epsilon),
            &mut callback,
        );
    }
    results.into_vec()
...
pub fn maxsim_search<'r, 'b: 'r, R: RelationRead, O: Operator>(
    _index: &'r R,
    _vector: <O::Vector as VectorOwned>::Borrowed<'_>,
    _probes: Vec<u32>,
    _epsilon: f32,
    mut _threshold: u32,
    _bump: &'b impl Bump,
    mut _prefetch_h1_vectors: impl PrefetcherHeapFamily<'r, R>,
    mut _prefetch_h0_tuples: impl PrefetcherSequenceFamily<'r, R>,
) -> (
    Vec<(
