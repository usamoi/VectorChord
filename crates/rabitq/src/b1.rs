...
pub fn code(vector: &[f32]) -> Code {
    crate::extended::code::<1>(vector)
}

pub mod binary {
    pub fn pack_code(input: &[u8]) -> Vec<u64> {
        crate::extended::pack_code::<1>(input)
            .into_iter()
            .flatten()
            .collect()
    }
    ...
    pub fn accumulate(lhs: &[u64], rhs: &[Vec<u64>; 4]) -> u32 {
        crate::extended::accumulate::<1, 4>(lhs, rhs)
    }
    ...
    pub fn half_process_dot(
        ...
        let rough = crate::extended::half_process_dot::<1, BITS>(n, value, code, lut);
        (rough,)
    }
    ...
    pub fn half_process_l2(
        ...
        let rough = crate::extended::half_process_l2::<1, BITS>(n, value, code, lut);
        (rough,)
    }
}
