...
impl<'a> VertexTupleReader<'a> {
    ...
    pub fn pointer(&mut self) -> Pointer {
        self.header.pointer
    }
}
...
impl WithReader for VectorTuple {
    ...
    fn deserialize_ref(source: &[u8]) -> VectorTupleReader<'_> {
        let checker = RefChecker::new(source);
...
impl WithWriter for VectorTuple {
    ...
    fn deserialize_mut(source: &mut [u8]) -> VectorTupleWriter<'_> {
        let mut checker = MutChecker::new(source);
...
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct VectorTupleReader<'a> {
    header: &'a VectorTupleHeader,
    elements: &'a [u8],
    neighbours: &'a [Neighbour],
}

impl<'a> VectorTupleReader<'a> {
    pub fn metadata(self) -> [f32; 3] {
        self.header.metadata
    }
    pub fn payload(self) -> Option<NonZero<u64>> {
...
impl VectorTupleWriter<'_> {
    ...
    #[expect(dead_code)]
    pub fn elements(&mut self) -> &mut [u8] {
        self.elements
    }
