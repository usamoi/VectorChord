...
use rabitq::b2::binary::{BinaryLut, BinaryLutMetadata};
use simd::Floating;
use std::fmt::Debug;
...
pub trait Vector: VectorOwned {
    ...
    fn binary_preprocess(vector: Self::Borrowed<'_>) -> BinaryLut;
...
pub trait Operator: 'static + Debug + Copy {
    ...
    fn binary_access<F: Call<u32, CodeMetadata, BinaryLutMetadata>>(
        lut: &BinaryLut,
        f: F,
    ) -> impl FnMut([f32; 4], &[u8]) -> F::Output;

    fn binary_process(n: u32, value: u32, code: CodeMetadata, lut: BinaryLutMetadata) -> (f32,);

    fn build(vector: <Self::Vector as VectorOwned>::Borrowed<'_>) -> rabitq::b2::Code;
}
...
impl Operator for Op<VectOwned<f32>, L2> {
    ...
    fn binary_access<F: Call<u32, CodeMetadata, BinaryLutMetadata>>(
        ...
        move |metadata: [f32; 4], elements: &[u8]| {
            let value = rabitq::b2::binary::accumulate(elements, &lut.1);
            f.call(value, CodeMetadata::from_array(metadata), lut.0)
        }
    }

    fn binary_process(n: u32, value: u32, code: CodeMetadata, lut: BinaryLutMetadata) -> (f32,) {
        rabitq::b2::binary::half_process_l2(n, value, code, lut)
    }

    fn build(vector: VectBorrowed<'_, f32>) -> rabitq::b2::Code {
...
impl Operator for Op<VectOwned<f16>, L2> {
    ...
    fn build(vector: VectBorrowed<'_, f16>) -> rabitq::b2::Code {
        Self::Vector::code(vector)
    }
}
