...
where
    ...
    P: Prefetcher<'b, T = Item<T>>,
{
    type Item = (Distance, NonZero<u64>);

    fn next(&mut self) -> Option<Self::Item> {
        while let Some(((.., AlwaysEqual(payload), AlwaysEqual(head)), list)) = self
            .prefetcher
...
pub fn rerank_index<'b, O: Operator, T, P: Prefetcher<'b, T = Item<T>>>(
    vector: O::Vector,
    prefetcher: P,
) -> Reranker<
    T,
...
pub fn rerank_heap<'b, O: Operator, T, P: Prefetcher<'b, T = Item<T>>>(
    vector: O::Vector,
    prefetcher: P,
