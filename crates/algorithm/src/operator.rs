...
#[derive(Debug)]
pub struct BlockAccessor<F>([u16; 32], F);
    ...
    > for BlockAccessor<F>
{
    ...
    fn finish(
        mut self,
        ((dis_u_2, factor_ppc, factor_ip, factor_err), delta): (
        ...
        std::array::from_fn(|i| {
            (self.1).call(
                self.0[i],
                (dis_u_2[i], factor_ppc[i], factor_ip[i], factor_err[i]),
                delta[i],
                lut,
            )
        })
...
impl Operator for Op<VectOwned<f32>, L2> {
    ...
    fn binary_access<F: Call>(
        ...
        let value = rabitq::binary::asymmetric_binary_dot_product(code.1, &lut.1) as u16;
...
impl<F: Fn(u16, (f32, f32, f32, f32), f32, (f32, f32, f32, f32)) -> R, R> Call for F {
    type Output = R;
