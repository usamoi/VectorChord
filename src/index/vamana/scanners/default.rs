...
use std::cmp::Reverse;
use std::num::NonZero;
use vamana::operator::{self, Dot, L2};
...
impl SearchBuilder for DefaultBuilder {
        ...
        R::Page: Page<Opaque = vamana::Opaque>,
    {
        ...
        let iter: Box<dyn Iterator<Item = (Reverse<Distance>, AlwaysEqual<NonZero<u64>>)>> =
        ...
        let iter = if let Some(threshold) = threshold {
            Box::new(iter.take_while(move |(Reverse(x), _)| x.to_f32() < threshold))
        } else {
            iter
        };
        ...
        Box::new(iter.map(move |(Reverse(distance), AlwaysEqual(pointer))| {
