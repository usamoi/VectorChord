...
pub fn mark(relation: impl RelationWrite, freepage_first: u32, pages: &[u32]) {
    ...
    while pages.is_empty() {
        ...
        let mut freespace_guard = relation.write(current, false);
        ...
        if freespace_guard.get_opaque().next == u32::MAX {
            let extend = relation.extend(false);
...
pub fn fetch(relation: impl RelationWrite, freepage_first: u32) -> Option<u32> {
    let first = freepage_first;
    assert!(first != u32::MAX);
    let (mut current, mut offset) = (first, 0_u32);
    loop {
